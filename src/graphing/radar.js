...
const radarFunc = function (size, radar) {
  ...
  function triangle(blip, x, y, order, group) {
    return group
      ...
      .attr(
        "d",
        `M412.201,311.406c0.021,0,0.042,0,0.063,0c0.067,0,0.135,0,0.201,0c4.052,0,6.106-0.051,
      ...
      .attr(
        "transform",
        `scale(${blip.width / NUMBER_34})${TRANSLATE}(${
          NUMBER_NEG_404 + x * (NUMBER_34 / blip.width) - NUMBER_17
        }, 
                            ${
                              NUMBER_NEG_282 +
                              y * (NUMBER_34 / blip.width) -
                              NUMBER_17
                            })`
      )
      .attr("class", order);
  ...
  function triangleLegend(x, y, group) {
    return group
      ...
      .attr(
        "transform",
        `scale(${NUMBER_22 / NUMBER_64}) ${TRANSLATE}(${
          NUMBER_NEG_404 + x * (NUMBER_64 / NUMBER_22) - NUMBER_17
        }, 
                            ${
                              NUMBER_NEG_282 +
                              y * (NUMBER_64 / NUMBER_22) -
                              NUMBER_17
                            })`
      );
  }
  ...
  function circle(blip, x, y, order, group) {
    return (group || svg)
      ...
      .attr(
        "d",
        `M420.084,282.092c-1.073,0-2.16,0.103-3.243,0.313c-6.912,1.345-13.188,8.587-11.423,16.874c1.732,
  ...
  function calculateBlipCoordinates(
    ...
    const radius = chance.floating({
      min: minRadius + blip.width / NUMBER_2,
      max: maxRadius - blip.width / NUMBER_2,
    });
    let angleDelta =
      (Math.asin(blip.width / NUMBER_2 / radius) * NUMBER_180) / Math.PI;
    angleDelta = angleDelta > NUMBER_45 ? NUMBER_45 : angleDelta;
    const angle = toRadian(
      chance.integer({ min: angleDelta, max: NUMBER_90 - angleDelta })
    );
  ...
  function plotBlips(quadrantGroup, rings, quadrantWrapper) {
    ...
    d3.select(QUADRANT_TABLE + order)
      .append("h2")
  ...
  function drawBlipInCoordinates(
    ...
    const blipText = `${blip.number()}. ${blip.name()}${
    ...
    const clickBlip = function () {
      ...
      const dialogBox = dialogContainer
        .append("div")
        .style("width", "400px")
        .style("padding", "20px")
      ...
      document.body.appendChild(dialogContainer.node());
  ...
  function createHomeLink(pageElement) {
    if (pageElement.select(HOME_LINK).empty()) {
      pageElement
        ...
        .attr(
          "d",
          `M27.6904224,13.939279 C27.6904224,13.7179572 27.6039633,13.5456925 27.4314224,13.4230122 L18.9285959,
  ...
  function redrawFullRadar() {
    ...
    tip.hide();
    d3.selectAll(BLIP_LINK).attr("opacity", NUMBER_1);
    ...
    d3.selectAll(QUADRANT_GROUP).style(POINTER_EVENTS, "auto");
  }
  ...
  function plotRadarHeader() {
    ...
    header
      ...
      .html(
        `<a href="${window.location.href}"><em class="bc-icon bc-lg ng-star-inserted">&#xea50;</em></a>`
      );
  ...
  function plotRadarFooter() {
    ...
    d3.select("#footer")
      ...
      .html(
        '<em class="bc-icon bc-lg ng-star-inserted">&#xea50;</em><p>Copyright © 2023 Grupo Bancolombia.</p></div>'
      );
  }
  ...
  function selectQuadrant(order, startAngle) {
    ...
    const translateX =
      ((-1 * (1 + adjustX) * size) / NUMBER_2) * (scale - 1) +
      -adjustX * (1 - scale / NUMBER_2) * size;
    const translateY =
      -1 * (1 - adjustY) * (size / NUMBER_2 - NUMBER_7) * (scale - 1) -
      ((1 - adjustY) / NUMBER_2) * (1 - scale / NUMBER_2) * size;

    const translateXAll =
      (((1 - adjustX) / NUMBER_2) * size * scale) / NUMBER_2 +
      ((1 - adjustX) / NUMBER_2) * (1 - scale / NUMBER_2) * size;
    const translateYAll =
      (((1 + adjustY) / NUMBER_2) * size * scale) / NUMBER_2;

    const moveRight =
      ((1 + adjustX) * (NUMBER_0_8 * window.innerWidth - size)) / NUMBER_2;
    const moveLeft =
      ((1 - adjustX) * (NUMBER_0_8 * window.innerWidth - size)) / NUMBER_2;
    ...
    d3.selectAll(`${QUADRANT_GROUP_}${order} .blip-link text`).each(
      function () {
  ...
  function plotRingDescription() {
    const ringDescription = d3
      .select("body")
      .append("div")
      .attr("id", "ringDescription")
      .style("width", "50%");
    ...
    span1.append('h4').text('Adoptar').style('display', 'inline')
    span1.append('p').text(`Servicios y capacidades habilitadas para uso por parte de los proyectos.`)

    span2.append('h4').text('Evaluar').style('display', 'inline')
    span2.append('p').text(`Servicios y capacidades habilitadas que requieren validación del caso de uso 
                            con el equipo de arquitectura de la CBO (Cloud Business Office).`)

    span3.append('h4').text('Prueba').style('display', 'inline')
    span3.append('p').text(`Servicios, Tecnologías o Plataformas que actualmente se encuentran en exploración o 
                            pendientes por aprobación por parte del equipo de arquitectura de la CBO (Cloud Business Office).`)

    span4.append('h4').text('Retener').style('display', 'inline')
    span4.append('p').text(`Servicios y capacidades no habilitados al ser reemplazados por otras 
                            funcionalidades disponibles en el portafolio de servicios del proveedor Cloud
                            o por disposiciones del equipo de arquitectura de la CBO (Cloud Business Office).`)
    */
  }
